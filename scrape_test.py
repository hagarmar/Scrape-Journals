...
def get_all_data_issue_view(website_url):
	...
	for article_type in ARTICLE_TYPES:
		...
		for article in all_articles_of_type:
			...
			date_format = datetime.date(int(article_date_raw[2]), MONTHS[article_date_raw[1]], int(article_date_raw[0]))

			# get journal name
			journal_name = article.find("abbr").string

			# put all in list of dicts
			all_articles_in_view.append({"Title":full_title, 
										 "Type":article_type, 
...
def scrape_Science_db(n_of_issues):
	...
	db = connection.journals.authors
